...
class TenantAwareCacheHandler(CacheHandler):
    ...
    def __getitem__(self, alias):
        ...
        if (
            not tenant_alias == constants.BASE_TENANT
            and alias in self._manager.template_aliases
        ):
            cache_alias = construct_cache_alias(
                tenant_alias=tenant_alias, template_alias=alias
            )
        logger.debug(
    ...
    def __delitem__(self, key):
        logger.debug(
            "Called del on {cls_name} with key {key}".format(
                cls_name=self.__class__.__name__, key=key
            )
        )
        if key in self:
